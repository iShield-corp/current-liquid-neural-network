...
class HomeostaticSTDP(STDPRule):
    ...
    def __init__(
        ...
        w_min: float = 0.0,
        w_max: float = 1.0,
        target_rate: float = 0.1,  # Target firing rate (spikes per time step)
        homeostatic_rate: float = 0.001,  # Rate of homeostatic adjustment
        tau_homeostatic: float = 1000.0  # Time constant for homeostatic plasticity
    ):
        super().__init__(learning_rate, tau_plus, tau_minus, a_plus, a_minus, w_min, w_max)
    ...
    def compute_weight_update(
        ...
        current_pre_rate = pre_spikes.mean(dim=(0, 1))  # [n_pre]
        current_post_rate = post_spikes.mean(dim=(0, 1))  # [n_post]
        ...
        # Homeostatic component: adjust weights to maintain target firing rates
        # If post-synaptic neuron fires too much, reduce all incoming weights
        # If it fires too little, increase all incoming weights
        post_rate_error = self.avg_post_rate - self.target_rate  # [n_post]
        homeostatic_update = -self.homeostatic_rate * post_rate_error.unsqueeze(0)  # [1, n_post]
        homeostatic_update = homeostatic_update * weights  # Proportional to current weight
...
class BCMRule(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('theta', None)  # Modification threshold
    ...
    def compute_weight_update(
        ...
        avg_pre = pre_activity.mean(dim=1)  # [batch, n_pre]
        avg_post = post_activity.mean(dim=1)  # [batch, n_post]
        
        # Update sliding threshold (moving average of squared post-synaptic activity)
        alpha = dt / self.tau_threshold
        current_post_squared = (avg_post ** 2).mean(dim=0)  # [n_post]
        self.theta = (1 - alpha) * self.theta + alpha * current_post_squared
        
        # BCM weight update
        # Δw = η * φ(post) * pre, where φ(post) = post * (post - θ)
        post_deviation = avg_post - self.theta.unsqueeze(0)  # [batch, n_post]
        phi_post = avg_post * post_deviation  # [batch, n_post]
        
        # Compute weight update: outer product of pre and φ(post)
...
class STDPLayer(nn.Module):
    ...
    def __init__(
        ...
        # Synaptic weights (initialized similar to Linear layer)
    ...
    def update_weights_stdp(
        ...
        with torch.no_grad():
            ...
            if self.track_eligibility:
                # Exponential decay of eligibility
                self.eligibility_trace *= 0.95
                self.eligibility_trace += weight_update
                
                # Use eligibility trace for actual update
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            if layers_to_enhance is None or any(target in name for target in layers_to_enhance):
                ...
                stdp_layer.weight.data = module.weight.data.clone()
                if module.bias is not None:
                    stdp_layer.bias.data = module.bias.data.clone()
                
                # Replace module (need to access parent)
                # This is simplified - actual implementation would need proper module replacement
...
if __name__ == "__main__":
    print("Testing STDP implementations...")
    
    # Create sample spike trains
    batch_size, time_steps, n_neurons = 4, 100, 50
    pre_spikes = (torch.rand(batch_size, time_steps, n_neurons) > 0.95).float()
    post_spikes = (torch.rand(batch_size, time_steps, n_neurons) > 0.95).float()
    weights = torch.rand(n_neurons, n_neurons) * 0.5
    
    # Test Classical STDP
    print("\n1. Classical STDP")
    classical = ClassicalSTDP()
    update = classical.compute_weight_update(pre_spikes, post_spikes, weights)
    print(f"   Weight update range: [{update.min():.6f}, {update.max():.6f}]")
    
    # Test Triplet STDP
    print("\n2. Triplet STDP")
    triplet = TripletSTDP()
    update = triplet.compute_weight_update(pre_spikes, post_spikes, weights)
    print(f"   Weight update range: [{update.min():.6f}, {update.max():.6f}]")
    
    # Test Homeostatic STDP
    print("\n3. Homeostatic STDP")
    homeostatic = HomeostaticSTDP()
    update = homeostatic.compute_weight_update(pre_spikes, post_spikes, weights)
    print(f"   Weight update range: [{update.min():.6f}, {update.max():.6f}]")
    print(f"   Avg post rate: {homeostatic.avg_post_rate.mean():.4f}")
    
    print("\n✅ All STDP variants working!")